...
| `ssl_clients`                             | SSL client certificates to generate (see [SSL Clients](#ssl-clients)) | `[]`                                                                 |
...
| `ssl_keys_clients_dir`                    | Client private key directory                                          | See [SSL Directories](#ssl-directories)                              |
| `ssl_keys_clients_mode`                   | File mode for client private keys                                     | `"0600"`                                                             |
| `ssl_keys_clients_owner`                  | Owner for client private keys                                         | `root`                                                               |
| `ssl_keys_clients_group`                  | Group for client private keys                                         | `root`                                                               |
...
| `ssl_certs_clients_dir`                   | Client certificate directory                                          | See [SSL Directories](#ssl-directories)                              |
| `ssl_certs_clients_mode`                  | File mode for client certificates                                     | `"0644"`                                                             |
| `ssl_certs_clients_owner`                 | Owner for client certificates                                         | `root`                                                               |
| `ssl_certs_clients_group`                 | Group for client certificates                                         | `root`                                                               |
...
| `ssl_keys_clients_dir`  | Client private key directory | `/etc/ssl/private/clients`         | `/etc/pki/tls/private/clients`     |
...
| `ssl_certs_clients_dir` | Client certificate directory | `/etc/ssl/certs/clients`           | `/etc/pki/tls/certs/clients`       |

## SSL Clients

The role can generate client certificates and keys signed by the same CA as the server certificate.
Each client is defined by its `name` (used as the certificate and key file basename) and `common_name` (used in the certificate subject).
Optional `extensions` can be added to specify additional extended key usages; the role always includes `clientAuth` and deduplicates any provided extensions
...
This generates `{{ ssl_keys_clients_dir }}/docker.key` and `{{ ssl_certs_clients_dir }}/docker.crt`.

## Copy CA Certificate and Key

When both `ssl_certs_ca_src` and `ssl_keys_ca_src` are set, the existing CA certificate and key are copied instead of being generated
